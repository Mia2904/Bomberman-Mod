...
public plugin_cfg()
{
	...
	g_menuintro = menu_create("MENU_INTRO_TITLE", "menu_intro");
	menu_additem(g_menuintro, "MENU_INTRO_JOIN");
	menu_additem(g_menuintro, "MENU_INTRO_SPECT");
	menu_additem(g_menuintro, "MENU_INTRO_INFO");
	menu_setprop(g_menuintro, MPROP_EXIT, MEXIT_NEVER);

	g_menuconfirm = menu_create("MENU_CONFIRM_TITLE", "menu_confirm");
	menu_additem(g_menuconfirm, "MENU_CONFIRM_YES");
	menu_additem(g_menuconfirm, "MENU_CONFIRM_NO");
	menu_setprop(g_menuconfirm, MPROP_EXIT, MEXIT_NEVER);

	g_menuinfo = menu_create("MENU_INFO_TITLE", "menu_info");
	menu_additem(g_menuinfo, "MENU_INFO_CHOOSE_ROOM");
	menu_additem(g_menuinfo, "MENU_INFO_HOW_TO");
	menu_additem(g_menuinfo, "MENU_INFO_ITEMS");
	menu_additem(g_menuinfo, "MENU_INFO_THROW");
	menu_additem(g_menuinfo, "MENU_INFO_ABOUT");
	menu_additem(g_menuinfo, "MENU_OPT_BACK");
...
public clcmd_say_team(id)
{
	...
	for (new i = 1; i <= 32; i++)
	{
		...
		if (g_battle[i] == room)
			print_color(i, "^4%L^3 %s^1 : %s", i, "CHAT_ROOM", g_battle[id], name, arg);
	}
...
public task_show_scores()
{
	...
	for (i = 3; i >= 0; i--)
	{
		...
		show_dhudmessage(players[i], "%L:%s", players[i], "HUD_SCORE_TITLE", msg);
	}
}
...
show_menu_confirm(id)
{
	...
	formatex(szMenuItem, charsmax(szMenuItem), "\y%L^n%L", id, "MENU_TAG", id, "MENU_CONFIRM_TITLE");
	menu_setprop(g_menuconfirm, MPROP_TITLE, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_CONFIRM_YES");
	menu_item_setname(g_menuconfirm, 0, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L^n", id, "MENU_CONFIRM_NO");
	menu_item_setname(g_menuconfirm, 1, szMenuItem);
...
show_menu_intro(id)
{
	...
	formatex(szMenuItem, charsmax(szMenuItem), "\y%L^n%L", id, "MENU_TAG", id, "MENU_INTRO_TITLE");
	menu_setprop(g_menuintro, MPROP_TITLE, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_INTRO_JOIN");
	menu_item_setname(g_menuintro, 0, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L^n", id, "MENU_INTRO_SPECT");
	menu_item_setname(g_menuintro, 1, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_INTRO_INFO");
	menu_item_setname(g_menuintro, 2, szMenuItem);
...
show_menu_info(id)
{
	...
	formatex(szMenuItem, charsmax(szMenuItem), "\y%L^n%L", id, "MENU_TAG", id, "MENU_INFO_TITLE");
	menu_setprop(g_menuinfo, MPROP_TITLE, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_INFO_CHOOSE_ROOM");
	menu_item_setname(g_menuinfo, 0, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_INFO_HOW_TO");
	menu_item_setname(g_menuinfo, 1, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_INFO_ITEMS");
	menu_item_setname(g_menuinfo, 2, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L^n", id, "MENU_INFO_THROW");
	menu_item_setname(g_menuinfo, 3, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L^n", id, "MENU_INFO_ABOUT", id, "MENU_TAG");
	menu_item_setname(g_menuinfo, 4, szMenuItem);

	formatex(szMenuItem, charsmax(szMenuItem), "%L", id, "MENU_OPT_BACK");
	menu_item_setname(g_menuinfo, 5, szMenuItem);
